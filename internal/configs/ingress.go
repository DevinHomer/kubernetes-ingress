...
func generateJWTConfig(owener runtime.Object, secretRefs map[string]*secrets.SecretReference, cfgParams *ConfigParams,
	...
	if secret.Error != nil {
		warnings.AddWarningf(owener, "JWK secret %s is invalid: %v", cfgParams.JWTKey, secret.Error)
	} else if secret.Type != secrets.SecretTypeJWK {
		warnings.AddWarningf(owener, "JWK secret %s is of a wrong type '%s', must be '%s'", cfgParams.JWTKey, secret.Type, secrets.SecretTypeJWK)
	}
